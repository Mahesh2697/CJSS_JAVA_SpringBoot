...
public class StudentData {
    public static void main(String args[]) {
        ...
        studentDetailsList.stream().filter(x-> x.getYear().equals("1stYear")).forEach(y-> {
            System.out.println(y.getId()+" "+y.getName()+" "+y.getBranch()+" "+y.getYear());
        });
        ...
        studentDetailsList.stream().forEach(g-> {
            studentFee.stream().filter(j-> g.getId()==j.getId()).forEach(m-> {
                System.out.println(g.getId()+" "+g.getName()+" "+g.getBranch()+" "+m.getFee()+" "+m.getDateOfJoining());
            });
        });
        ...
        System.out.println("----------------using-Streams-------------------");
        studentDetailsList.stream().forEach(r-> {
            studentMarks.stream().forEach(o-> {
                subjects.stream().filter(i->  r.getId()==o.getId()).forEach(p-> {
                    System.out.println(r.getId()+" "+r.getBranch()+" "+p.getSubName()+" "+o.getMarks());
                });
            });
        });
        ...
        studentDetailsList.stream().sorted(Comparator.comparing(StudentDetails::getYear)).collect(Collectors.toList()).stream().forEach(f->{
            subjects.stream().forEach(h-> {
                studentMarks.stream().filter(u-> f.getId()==u.getId()).forEach(k-> {
                    System.out.println(f.getId()+ " " +f.getName()+ " " +h.getSubName()+ " " +k.getMarks()+ " " +f.getYear());
                });
            });
        });
